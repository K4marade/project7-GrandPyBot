...
from gpapp.utils.constants import GRANDPY_FIRST_ANSWER,\
    GRANDPY_SECOND_ANSWER, GRANPY_WRONG_QUESTION
...
class GrandPy:
    ...
    def __init__(self):
        """
        Class constructor with GrandPy's preset data
        and answers in a dictionary
        """
        self.parser = Parser()
        self.wiki = Wiki()
        self.gmaps = Gmaps()
        self.answer = {
            ...
            'no_wiki_info': "Désolé mon petit, je n'ai pas réussi à trouver "
                            "d'informations sur ce lieu mais voici "
                            "l'adresse : "
        }

    def grandpy_answer(self, user_message: str) -> dict:
        """
        Method that sets new values to GrandPy's dictionary
        to return one of GrandPy's answer with possible place's information
        and location according to parsed user's input
        """
    ...
    def search_wiki(self, keywords: str):
        """
        Method that returns a possible list
        with place information from Wikipedia according to user's input
        """

        try:
            # Place's page ID
            page_id = self.wiki.get_page_id(keywords)
            # Place's intro information
    ...
    def search_google(self, keywords: str):
        """
        Method that returns a possible list with place's location
        with the address, latitude and longitude, according to user's input
        """
